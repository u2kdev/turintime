...
def parse_all(raw: dict) -> dict:
    ...
    for card in rows("cards"):
        ...
        teacher_name = ", ".join(
            teachers[tid].get("short", tid)
            for tid in teach_ids if tid in teachers
        )
        room_name = ", ".join(
            classrooms[rid].get("name", rid)
            for rid in room_ids if rid in classrooms
        )
            ...
            if "regularttGetData" in r.url:
                try:
                    ...
                    if not data.get("reload"):
                        ...
                        print("[OK] Данные перехвачены!")
                except:
                    pass

        page.on("response", on_response)
        print("[BROWSER] Загружаем страницу TTPU...")
...
def init_db():
    ...
    conn.executescript("""
        CREATE TABLE IF NOT EXISTS groups (
            id TEXT PRIMARY KEY, name TEXT, short TEXT, color TEXT
        );
        ...
        CREATE TABLE IF NOT EXISTS lessons (
            ...
            subject_name TEXT, subject_short TEXT,
            teacher_name TEXT, room_name TEXT,
            day TEXT, day_idx INTEGER, period INTEGER,
